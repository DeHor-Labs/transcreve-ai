...
- Quando a retencao duravel for pedida, rode `transcreveai share RUN_ID --json` depois da analise. Se o run usou indice isolado, passe o mesmo `--index-db` ou use `transcreveai share --run-dir "$RUN_DIR" --json`. O pacote gerado inclui `handoff.md`, `manifest.json`, `knowledge.md` e `analysis.json`, e atualiza `catalog.json`/`index.md` no diretorio raiz de compartilhamento para outro agente descobrir e retomar sem depender de paths soltos.
