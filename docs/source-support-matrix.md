...
| Instagram / Reels / Posts | `yt-dlp` diretamente da URL do reel; posts/carrosséis usam fallback de `/embed/` para baixar todos os itens visíveis do carrossel como mídias sequenciais. | Falhas de parser/carregamento: coletar URL de origem (`?igsh=...` limpo), reprocessar com cookies reais do navegador (`--cookies-browser chrome`), manter fallback para arquivo local se disponibilizado pelo usuário. Se o embed não expuser mídia, usar Open Graph como preview único com aviso. Legendas: normalmente ausentes; não há uso atual no pipeline. | Forte bloqueio anti-bot e mudanças de login, conteúdo privado/age-limited, instabilidade em contas com segurança agressiva; carrosséis mistos com vídeo/imagem podem ter limites de áudio por item. | `transcreveai analyze "https://www.instagram.com/p/..." --cookies-browser chrome --ai off --max-frames 20` | 🟡 MVP com ressalvas |
...
- Carrossel/slideshow é tratado como tipo de mídia transversal: se qualquer adapter
  retornar múltiplas mídias sequenciais, o dossiê usa estrutura por slide em vez de
  timeline de vídeo. O fallback `/embed/` é específico de Instagram, não do modelo de
  dossiê.
